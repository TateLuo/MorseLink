...
from utils.mqtt_query_tool import AsyncMQTTQueryTool
...
class QSOOline(QWidget):
    ...
    def startQuery(self):
        ...
        url = "http://你的ip"
        port = 18083
        api_path = "/api/v5/clients"
        username = "账号"
        password = "密码"


        # 创建异步查询工具对象
        self.worker = AsyncMQTTQueryTool(
            url, port, api_path, username, password
        )
        self.worker.data_fetched.connect(self.displayData)
        self.worker.start()

    def displayData(self, data):
        """
        显示查询结果，并解析客户端数量。
        :param data: 查询结果（字典格式）
        """
        try:
            # 如果数据中包含错误信息，直接显示
            if "error" in data:
                self.label_number_clients.setText(f"错误: {data['error']}")
                return

            # 解析 count 字段
            count = data.get("meta", {}).get("count", "未知")
            formatted_data = json.dumps(data, indent=2, ensure_ascii=False)
            result = f"当前客户端数量: {count}\n\n完整数据:\n{formatted_data}"
            self.label_number_clients.setText(f'服务器在线人数：{count}')

        except Exception as e:
            self.label_number_clients.setText(f"解析数据失败: {str(e)}")
