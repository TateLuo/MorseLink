...
def _tr(text: str) -> str:
    return QCoreApplication.translate("MorseCodeTranslator", text)
...
class MorseCodeTranslator:
    ...
    def letter_to_morse(self, morse_code):
        return self.morse_code_dict.get(morse_code, _tr("未知"))
    ...
    def morse_to_text(self, morse_code):
        ...
        for word in words:
            if word:  # 确保word不为空
                letters = word.split('/')
                decoded_word = ''.join(self.morse_code_dict.get(letter, _tr("未知")) for letter in letters if letter)
